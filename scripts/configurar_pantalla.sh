...
auto_config="$script_dir/../config/xorg-auto.conf"
...
usage() {
    cat <<'EOF'
Uso: configurar_pantalla.sh [--check] [--restart-ui] [--force] [--vesa] [--config-only]

Compila e instala el controlador comunitario SiS 671 compatible con Xorg 21,
habilita aceleracion EXA 2D y configura el panel 1280x800 sin forzar VGA.
MergedFB auto solicita extension si detecta VGA al iniciar Xorg.
La conexion/desconexion en caliente aun no esta validada.

  --config-only Reutiliza el controlador instalado; no descarga ni compila.
  --check       Solo comprueba la configuracion guardada (no el estado en vivo).
...
if $check_only; then
    if $use_vesa; then
        ...
        [[ -r $driver_module ]] && cmp -s "$auto_config" "$xorg_config" \
            && { echo 'OK: panel y autodeteccion al iniciar Xorg configurados; hotplug sin validar.'; exit 0; }
        echo 'PENDIENTE: falta el controlador o la configuracion de autodeteccion.'
    fi
    exit 1
fi
...
EOF
    ...
    elif [[ ! -s $driver_module ]]; then
        echo 'Falta el controlador SiS; ejecuta sin --config-only.' >&2
        exit 1
    fi
    cp -- "$auto_config" "$config_tmp"
fi
...
if $use_vesa; then
    ...
    echo "Configuracion SiS instalada: LCD y deteccion opcional de VGA al iniciar Xorg."
    echo "Extension y conexion/desconexion en caliente pendientes de validar con monitor."
fi
