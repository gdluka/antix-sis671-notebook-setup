...
for required in configurar_pantalla.sh configurar_arranque_visual.sh configurar_rofi.sh configurar_firefox.sh configurar_psd.sh; do
...
echo '[1/6] Comprobando permisos administrativos...'
sudo -v

echo '[2/6] Actualizando repositorios y paquetes...'
sudo apt-get update
sudo apt-get upgrade -y

echo '[3/6] Instalando paquetes necesarios...'
...
echo '[4/6] Configurando aplicaciones del usuario...'
...
echo '[5/6] Aplicando el arreglo de pantalla despues de la actualizacion...'
screen_args=()
$restart_ui && screen_args+=(--restart-ui)
"$script_dir/configurar_pantalla.sh" "${screen_args[@]}"

echo '[6/6] Configurando GRUB y el arranque silencioso...'
