...
for required in configurar_pantalla.sh configurar_arranque_visual.sh configurar_rofi.sh configurar_touchpad.sh configurar_firefox.sh configurar_psd.sh configurar_agente_impresion.sh; do
...
echo '[1/8] Comprobando permisos administrativos...'
sudo -v

echo '[2/8] Actualizando repositorios y paquetes...'
sudo apt-get update
sudo apt-get upgrade -y

echo '[3/8] Instalando paquetes necesarios...'
...
echo '[4/8] Configurando aplicaciones del usuario...'
...
echo '[5/8] Reforzando el inicio del agente de impresion...'
...
echo '[6/8] Aplicando el arreglo de pantalla despues de la actualizacion...'
screen_args=()
$restart_ui && screen_args+=(--restart-ui)
"$script_dir/configurar_pantalla.sh" "${screen_args[@]}"

echo '[7/8] Configurando GRUB y el arranque silencioso...'
"$script_dir/configurar_arranque_visual.sh"

echo '[8/8] Configurando energía e hibernación...'
